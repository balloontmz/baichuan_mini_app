...
function yd_tree_select_render() {
    ...
    function format_item(selectId, id, name){
	    var tpl = "<div class='d-inline-block border rounded align-self-center pl-1 pr-1 ts-select-item'>" +
            ...
            +name+"</span><span class='text-muted pointer' onclick='yd_ts_remove_item(\""+selectId+"\",\""+id+"\",this)'><i class='iconfont icon-delete-copy'></i></span></div>";
	    return tpl;
    }
    ...
    var render = function (el) {
        ...
        input += "<input type='text' id='" + inputId + "' placeholder='"
            +(placeholder||"请选择")+"' class='yd-tree-select-input flex-grow-1 m-0 border-0'/><span class='text-muted ts-remove-all pr-1' onclick='yd_ts_remove_all(\""+selectId+"\",\""+inputId+"\")'><i class='iconfont icon-reject'></i></span></div>";
