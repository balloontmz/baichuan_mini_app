...
<div class='m-3'>
    <div class="d-flex align-items-center justify-content-between">
        <h2 class="font-weight-bold">二级属性列表</h2>
        <div class="">
            <button type="button" class="layui-btn layui-btn-normal yd-dialog" data-url="/attribute/second_attribute/add">新增
            </button>
        </div>
    </div>
    ...
    <table class="layui-table" lay-data="{url:'/attribute/second_attribute/index.json', page:true, id:'test'}"
           lay-filter="test">
        <thead>
        <tr>
            <th lay-data="{field:'name'}">名称</th>
            <th lay-data="{field:'first_attribute_name'}">上级属性名称</th>
...
<script type="text/html" id="barDemo">
    <a class="layui-btn layui-btn-normal layui-btn-xs yd-dialog"
       data-url="/attribute/second_attribute/add?second_attribute_id={{d.id}}" lay-event="edit">修改</a>
        ...
        table.on('tool(test)', function (obj) {
            ...
            if (obj.event === 'del') {
                layer.confirm("确定要该分类吗？", function (index) {
                    $.ajax({
                        url: "/attribute/second_attribute/remove",
                        method: "post",
                        data: {
                            id: id
                        },
