...
<div class='m-3'>
    <div class="d-flex align-items-center justify-content-between">
        <h2 class="font-weight-bold">产品总分类列表</h2>
        <div class="">
            <button type="button" class="layui-btn layui-btn-normal yd-dialog" data-url="/product/product_type/add">新增
            </button>
        </div>
    </div>
    <table class="layui-table" lay-data="{ url:'/product/product_type/index.json', page:false, id:'test'}"
           lay-filter="test">
        <thead>
        <tr>
...
<script type="text/html" id="barDemo">
    <a class="layui-btn layui-btn-normal layui-btn-xs yd-dialog"
       data-url="/product/product_type/add?product_type_id={{d.id}}" lay-event="edit">修改</a>
        ...
        table.on('tool(test)', function (obj) {
            ...
            if (obj.event === 'del') {
                layer.confirm("确定要该分类吗？", function (index) {
                    $.ajax({
                        url: "/product/product_type/remove",
                        method: "post",
                        data: {
                            id: id
                        },
