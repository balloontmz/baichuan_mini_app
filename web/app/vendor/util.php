 ...
 */
function uuid() {
    return YZE_Model::uuid();
}

/**
 * 把数字0-6 转换成中文的星期几, 用date("w")获得是周几
 * 
 * @param  $d
 * @author leeboo
 * @return string
...
function dateFormatToString($date) {
    ...
    return date("Y-m-d 周" . $weeks[date("N", strtotime($date))] . "", strtotime($date));
}
/**
...
function find_by_uuid($class, $uuid, $include_deleted=false){
    return $uuid ? $class::from()->where(($include_deleted ? "" : "is_deleted=0 and ")." uuid=:uuid")->getSingle([":uuid"=>$uuid]) : null;
}
...
/**
 * 如果content 不为空,则用start end包尾并echo输出
 * @param   $content
 * @param   $start
 * @param   $end
 * @return string
 */
function wrap_output($content,$start,$end){
    echo $content ? $start.$content.$end : "";
}
...
class Lunar {
    ...
    /**
     * 将阴历转换为阳历   
     * @param string year 阴历-年
     * @param string month 阴历-月，闰月处理：例如如果当年闰五月，那么第二个五月就传六月，相当于阴历有13个月，只是有的时候第13个月的天数为0
     * @param string date 阴历-日
     * @return array
     */
    ...
    /**
     * 计算2个阳历日期之间的天数   
     * @param string year 阳历年
     * @param string cmonth
     * @param string cdate
     * @param string dmonth 阴历正月对应的阳历月份
     * @param string ddate 阴历初一对应的阳历天数
     * @return int
     */
    ...
    function getFestival($today, $nl_info = false, $config = 1) {
        ...
        if (!$nl_info)
            $nl_info = $this->convertSolarToLunar($y, intval($m), intval($d));

        if ($nl_info[7] > 0 && $nl_info[7] < $nl_info[4])
            $nl_info[4] -= 1;
        ...
        if ($m == 5 && ($d > 7) && ($d < 15) && ($glweek == 0))
            array_push($festivals, "母亲节");
        if ($m == 6 && ($d > 14) && ($d < 22) && ($glweek == 0))
            array_push($festivals, "父亲节");

        $jieqi = $this->getJieQi($y, $m, $d);
        if ($jieqi)
            array_push($festivals, $jieqi);
 ...
 * @return string 人脸特征点数据
 * @throw YZE_FatalException
 */
function getFaceFeature($image_path){
    return "";
    if(PHP_OS!='WINNT') return "";
    if ( ! file_exists(YZE_APP_PATH."FaceEngineApi.jar")){
        throw new \yangzie\YZE_FatalException(YZE_APP_PATH."FaceEngineApi.jar 文件不存在");
    }
    ob_start();
    $result = system("java -jar ".YZE_APP_PATH."FaceEngineApi.jar -f {$image_path} -id ".FACE_APP_ID." -key ".FACE_SDK_KEY);
    //var_dump("java -jar ".YZE_APP_PATH."FaceEngineApi.jar -f {$image_path} -id ".FACE_APP_ID." -key ".FACE_SDK_KEY);
    $result = ob_get_clean();
    $json   = json_decode($result,true);
    if ( ! $json || ! $json["feature"]) throw new YZE_FatalException("生成人脸特征点失败: {$result}");
    return $json["feature"];
}

/**
 * 根据link的后缀来判断点击后的效果，如果是dialog后缀，那么就对话框，其他情况则是页面重定向；
 * 两种方式都会在最顶层的windows中进行
 *
 * @param $link
 * @param $text
 * @return string
 */
function showNormalOrDialogButton($link, $text, $cls = "layui-btn-sm"){
    if(strtoupper( pathinfo( parse_url($link, PHP_URL_PATH), PATHINFO_EXTENSION))=="DIALOG"){
        return '<button data-backdrop="normal" data-size="large" 
                class="layui-btn layui-btn-default  '.$cls.' yd-dialog" 
                data-url="'.$link.'" data-in-top="1" type="button">'.$text.'</button>';
    }else{
        return '<button  class="layui-btn layui-btn-default '.$cls.' yd-link" 
                data-url="'.$link.'" data-target="_blank" data-in-top="1" type="button">'.$text.'</button>';
    }
}
...
function get_user_permissions(){
    ...
    $acls = YZE_Session_Context::get_instance()->get("ACLLIST");
    if ( $acls) return $acls;

    $acls = [];
    $sql = "select * from `option` where option_name like 'acl-%'";
    $rst = YZE_DBAImpl::getDBA()->nativeQuery2($sql);
    while ($rst->next()){
        $acls[ $rst->f("option_name") ] = json_decode( html_entity_decode( $rst->f("option_value") ), true);
    }


    $myAcls = ["deny"=>[], "allow"=>[]];
    foreach ($acls as $acl => $aclInfos){
        foreach ($aclInfos as $aclInfo){
            $inJobLevel = true;
            $inOrg = true;
            $inUser = true;
            if( $aclInfo['job_level'] && ! in_array( $loginUser->job_level, $aclInfo['job_level'] )){
                $inJobLevel = false;
            }
            if( $aclInfo['user_id'] && ! in_array( $loginUser->id, $aclInfo['user_id'] )){
                $inJobLevel = false;
            }
            if( $aclInfo['org_id'] && ! in_array( $loginUser->org_id, $aclInfo['org_id'] )){
                $inJobLevel = false;
            }

            if ($inJobLevel && $inOrg && $inUser){
                $myAcls['allow'][] = preg_replace("/^acl-/","", $acl);
            }
        }
    }

    YZE_Session_Context::get_instance()->set("ACLLIST", $myAcls);
    return $myAcls;
}
