...
function yze_autoload($class) {
    ...
    if ($_[0] == "app") {
        ...
        if (!file_exists($file)) {
            $file = YZE_INSTALL_PATH . strtr($class, array("\\" => "/")) . ".class.php";
        }
        if (!file_exists($file)) {
            $file = YZE_INSTALL_PATH . strtr($class, array("\\" => "/")) . ".php";
        }
    }
...
function yze_go($uri = null, $method = null, $return = null, $request_method = null) {
    ...
    $output_view = function($request, $controller, $response) {
    ...
    try {
        ...
        if (is_a($response, "\\yangzie\\YZE_View_Adapter")) {
            $view_result = $output_view($request, $controller, $response);
            if ($return)
                return $view_result;
            echo $view_result;
            return;
